...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        <div>
            ...
            <form action="{{ route( 'shopsUpdate', $shop['id'] ) }}" method = "POST">
                @csrf
                <label for="url">URL eshopu:</label><br>
                <input type="text" id="url" name="url" value="{{$shop['url']}}"><br>
                <label for="notes">Poznámky:</label><br>
                <textarea id="notes" name="notes" rows="4" cols="50">{{$shop['notes']}}</textarea><br>
